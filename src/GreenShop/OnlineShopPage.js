...
function OnlineShopPage(props) {
    ...
    const [ID, setID] = useState(params.get('no'));
    ...
    useEffect(() => {
        //根據ProjNO抓取後端api資料
        const uri = `${props.SSL}//${hostname}/APIs/StoresWeb2/${ID}`;
        if (ID)
            fetch(uri, {
                method: 'GET'
            })
                .then(res => {
                    ...
                    const uriRec1 = `${props.SSL}//${hostname}/APIs/StoresWeb`
                    console.log(uriRec1)
                    fetch(uriRec1, {
                        method: 'GET'
                    })
                        .then(res => {
                            ...
                            var removeIndex = result.Detail.map(function (item) { return String(item.Num); }).indexOf(ID);
    ...
    useEffect(() => {
        if (ID)
            fetch(`${props.SSL}//${hostname}/APIs/StoreGoods/${ID}`, {
                method: 'GET'
            }).then(res => {
                return res.json();
            }).then(result => {
                    ...
                    <CardDeck className="row shop">
                        <div ref={componentRef} style={{ marginTop: "40px" }} className="tour col-12 col-md-9 col-md-9 col-lg-9">
                                        ...
                                        <div className="col-sm-12 col-md-12 col-lg-8">
                                            <div className="content-outside-container">
                                                <div>
                                                    ...
                                                    <div className="">
                                                        <div className="content-name">
                                                            {fetchData.NameEN && <p className="">{fetchData.NameEN}</p>}
                                                            {fetchData.Type && <p className="">{fetchData.Type}</p>}
                                                        </div>
                                                ...
                                                <div className="bottom-content-wrapper">
                                                    ...
                                                    <SocialBtn myStyle='d-flex justify-content-end' handlePrint={handlePrint} url={`/categories/shop/detailPage?${ID}`} title={pageName} collectTypeId={collectTypeId} activityGuid={ID} collector={collector} history={history} />
                            ...
                            <>
                                <Card className="product shop-wrapper product-form-border content-card border-radius">
                                    <div className="">
                                        <h2 className="content-title with-border"><i className="fas fa-play" aria-hidden="true" alt="商店特色圖示"></i>&nbsp;商店特色</h2>
                                    ...
                                    <div className="">
                                        <h4 className="content-title with-border"><i className="fas fa-play" aria-hidden="true" alt="販賣環保產品圖示"></i>&nbsp;販賣環保產品</h4>
                                        <div className="resBtn-wrapper">
                        ...
                        {/* <div className="col-12 col-md-3 col-lg-3 sideCard-container mb-5">
                         ...
                         {fetchRecData.map((fetchRecData, index) =>
                            <Link onClick={() => setID(fetchRecData.StoreID)} title={fetchRecData.StoreName} key={`1_${index}`} as={Link} to={`/categories/greenProductdetailPage?product=${fetchRecData.Num}&type=${fetchRecData.Class}`}>
                                <Card className="mb-3 rec-card">
                                    <div>
                                        
                                        <img style={{ width: "100%" }} alt={fetchRecData.StoreName} title={fetchRecData.StoreName} src={fetchRecData.StorePic !== "" ? fetchRecData.StorePic : "../../images/blankLeef.png"} />
                                        
                                        <div><h6 className="rec-title">{fetchRecData.StoreName}</h6></div>
                                    </div>
                                </Card>
                            </Link>
                        )} 
...
export default withRouter(OnlineShopPage);