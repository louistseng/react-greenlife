...
function KnowledgeProcess(props) {
    ...
    useEffect(() => {
        window.scrollTo(0, 0)
        setLink()
        fetch(`${SSL}//${domain}/api/api/Manager/Knowledge/Record`, {
            method: 'POST',
            body: serialize({
                Guid: authGuid,
                Page: String(page),
            ...
            .then(res => {
                ...
                // console.log("歷程",result)
                if (result.isSucess) {
                    setPageCount(result.resultObject.pageCount)
                    setKnowledgeRecords(result.resultObject.records)
                }
            });
    ...
    const processTableData = (
        knowledgeRecords.map((data, index) => {
    ...
    return (
        <>
            <div className="col-12 col-lg-8">
                <h3 className="green-title">活動審核<i class="fas fa-chevron-circle-right" aria-hidden="true"></i>歷程</h3>
...
export default withRouter(KnowledgeProcess);