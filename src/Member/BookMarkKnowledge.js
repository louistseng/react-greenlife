...
function BookMarkKnowledge(props) {
    ...
    useEffect(() => {
        ...
        fetch(`${SSL}//${domain}/api/api/Member/Share/PageTag`, {
            ...
            .then(res => {
                ...
                const knowledgeBookMark = { typeId: 5, typeName: '知識綠上傳', count: pageCount, typeHref: null }
    ...
    useEffect(() => {
        ...
        fetch(`${SSL}//${domain}/api/api/Knowledge/MyList`, {
            method: 'POST',
            body: serialize({
                Page: String(page),
                Count: count
    ...
    function deleteArticle(deleteGuid) {
        ...
        if (comfirm) {
            fetch(`${SSL}//${domain}/api/api/Knowledge/delete`, {
                method: 'POST',
                body: serialize({
                    Guid: deleteGuid
                }),
    ...
    const handlePageClick = (data) => {
        ...
        history.push(`/member/BookMarkKnowledge?page=${data.selected + 1}&name=知識綠&link=/member/shareKnowledge&type=4`)
    };
        ...
        <>
            ...
            <div className="col-sm-12 col-md-8 col-lg-9">
                ...
                <div className="bookmark-wrapper-bookMark">
                    {bookMark.slice(0, 4).map((data, index) =>
                        <div
                            key={index}
                            onClick={() => {
                ...
                <div className="d-flex share-green-wapper">
                    {typeId === "4" ?
                        <Link to="/member/shareKnowledge"><button className="share-green-btn mt-2">發佈知識綠</button></Link>
                        :
                ...
                {fetchData ?
                    fetchData.map((fetchData, index) =>
                        <div key={index} className="book-mark-share-content">
                            <Link className="link-wrapper" to={`/member/KnowledgePage?${fetchData.guid}`}>
                                <div className="d-flex justify-content-start">
                                    <img src={String(fetchData.picPath).replaceAll("\\\\", "\\")} alt={fetchData.title} title="網誌圖片" />

                                    <div className="content-text">
                                        <h6>{formatDate(fetchData.startDatetime)}</h6>
                            ...
                            <div className="right-side-btn">
                                ...
                                <Link
                                    ...
                                    to={`/member/shareKnowledge?edit=${fetchData.guid}`} >
...
export default withRouter(BookMarkKnowledge);