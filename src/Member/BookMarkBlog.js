...
function BookMark(props) {
    ...
    useEffect(() => {
        window.scrollTo(0, 0)
        const urlBlog = `${SSL}//${domain}/api/api/Blog/MyList`
    ...
    function deleteArticle(deleteGuid) {
        ...
        if (comfirm) {
            fetch(`${SSL}//${domain}/api/api/Blog/${deleteGuid}`, {
                method: 'POST',
                headers: {
                    "Token": memberToken
                }
                ...
                .then(res => {
                    ...
                    clickRecord("F2D7AFDB-0842-460F-8F57-91926B1CF3B8", "25", collector)
                    history.go(0)
                });
        }
    }
    ...
    const handlePageClick = (data) => {
        ...
        history.push(`/member/BookMarkBlog?page=${data.selected + 1}&name=綠生活網誌&link=/member/shareBlog&type=3`)
    };
                ...
                {fetchData ?
                    fetchData.map((fetchData, index) =>
                        <div key={index} className="book-mark-share-content">
                            <Link className="link-wrapper" to={`/member/blogPage?${fetchData.guid}`}>
                            ...
                            <div className="right-side-btn">
                                ...
                                <Link
                                    ...
                                    to={`/member/edit_blog?${fetchData.guid}`} >
